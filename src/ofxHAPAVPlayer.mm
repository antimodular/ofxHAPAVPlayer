...
//--------------------------------------------------------------
void ofxHAPAVPlayer::drawSubsection(float x, float y, float w, float h,float sx, float sy, float sw, float sh){
    if(delegate == nil) return;
    if(bNeedsShader && !shader.isLoaded()) return;

    if(bNeedsShader) shader.begin();
    videoTextures[0].drawSubsection( x,  y, 0, w,  h,  sx,  sy,  sw,  sh);
    if(bNeedsShader) shader.end();
